...
#### Экспериментальный рейтинг силы

Сквозной рейтинг Glicko-2 включается параметром `?rating=1`. Выбор сохраняется
в `localStorage`, поэтому параметр нужен только при первом открытии. Выключить
эксперимент можно через `?rating=0`.

После включения рейтинг появляется в турнирной таблице, профилях игроков и на
странице `/rating`. Он вычисляется из всей истории матчей, не влияет на
официальные места и не требует отдельной миграции базы данных.
...
`DIGEST_CRON_SECRET`. Их значения передаются только в контейнеры x260.
